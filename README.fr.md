...
| **Woodpecker CI (k0s)** | Woodpecker v2.7.2 + k0s | Nœud 1 (CT 115 · Alpine 3.24) | `:8000` / `:9000` (gRPC) | Moteur CI/CD conteneurisé exécuté dans un micro-cluster Kubernetes k0s sur Alpine Linux |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **200** | `opnsense` | Hardened FreeBSD 14 | 4 Cœurs | 4 096 Mo | **2 048 Mo** | VirtIO Net Multi-VLAN | Pare-feu Périmétrique, Zenarmor NGFW (L7 Shun-Tuning), AdGuard Home + Unbound Split-DNS (:5335), FQ_CoDel Traffic Shaper, CrowdSec IPS + Threat Feeds, FRR BGP/OSPF, Découverte LLDP, iperf3, Sauvegarde Chiffrée Git/Nextcloud |
...
| **212** | `metasploitable2` | Metasploitable 2 (Ubuntu 8.04) | 1 Cœur | 512 Mo | **512 Mo** | VirtIO Net + IDE (8 Go) | Cible Linux Vulnérable pour Tests d'Intrusion & Calibrage IDS/IPS |
| **213** | `tpot-honeypot` | Debian 12 / T-Pot 24.04 | 4 Cœurs | 8 192 Mo (8 Go) | **4 096 Mo (4 Go)** | VirtIO Net + SCSI (60 Go) | Plateforme Leurre Multi-Honeypot (Cowrie, Dionaea, Honeytrap, Elastic, Kibana, Suricata) |
| **214** | `haiku` | Haiku R1/beta5 | 2 Cœurs | 2 048 Mo (2 Go) | **1 024 Mo (1 Go)** | e1000 + SCSI (20 Go) | Système d'Exploitation Modulaire Inspiré de BeOS, API Objet C++ & Système de Fichiers OpenBFS |
| **215** | `plan9` | Plan 9 from Bell Labs (9front) | 1 Cœur | 512 Mo (512 Mo) | **512 Mo** | e1000 + IDE (12 Go) | Système de Recherche des Laboratoires Bell, Protocole Distribué 9P & Espaces de Noms par Processus |
| **216** | `reactos` | ReactOS 0.4.16 | 1 Cœur | 1 024 Mo (1 Go) | **1 024 Mo** | e1000 + IDE (32 Go) | Architecture de Compatibilité Binaire Windows NT Open Source & Sous-Système Natif Win32 |
| **217** | `securityonion` | Security Onion 3.2 / Wazuh SIEM | 4 Cœurs | 8 192 Mo (8 Go) | **4 096 Mo (4 Go)** | VirtIO Net + SCSI (50 Go) | Plateforme SIEM d'Entreprise, HIDS, Analyse de Journaux, Surveillance Réseau (Zeek, Suricata, Elastic, Kibana) |
| **218** | `remnux` | REMnux v7 / Noble | 2 Cœurs | 4 096 Mo (4 Go) | **2 048 Mo (2 Go)** | VirtIO Net + SCSI (40 Go) | Distribution Linux Dédiée à l'Ingénierie Inverse, l'Analyse de Malware, la Forensique et le DFIR |
| **219** | `redox` | Redox OS 0.9.0 | 2 Cœurs | 2 048 Mo (2 Go) | **1 024 Mo (1 Go)** | e1000 + SCSI (10 Go) | Système d'Exploitation Micro-noyau Écrit en Rust, RedoxFS & Conception Inspirée de Minix/Plan 9 |
| **220** | `freedos` | FreeDOS 1.3 | 1 Cœur | 512 Mo (512 Mo) | **256 Mo** | e1000 + IDE (2 Go) | Environnement DOS Open Source, Assembleur x86 16 bits en Mode Réel & Systèmes Anciens |
